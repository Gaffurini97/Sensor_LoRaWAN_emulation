...
void send_measures(struct timeval *msg_sent,char* category){
	JSON_Value *s_root_value = json_value_init_object();
	JSON_Object *s_root_object = json_value_get_object(s_root_value);
	json_object_dotset_string(s_root_object, "msg.type", "MEASURES");
	json_object_dotset_string(s_root_object, "msg.ref.type", "GW");
	json_object_dotset_string(s_root_object, "msg.ref.id", gateway_ID);
	json_object_dotset_string(s_root_object, "msg.category", category);
	json_object_dotset_number(s_root_object, "msg.timestamp.value", msg_sent->tv_sec);
	json_object_dotset_number(s_root_object, "msg.timestamp.us", msg_sent->tv_usec);
	char* s_serialized_string = json_serialize_to_string(s_root_value);
	mqtt_publish(s_serialized_string,"/measures", 0);
	json_free_serialized_string(s_serialized_string);
	json_value_free(s_root_value);
}
...
static int parse_gateway_configuration(const char * conf_file) {
	...
	if (str != NULL) {
		gateway_ID = malloc(strlen(str)+1);
		memcpy(gateway_ID,str,strlen(str)+1);
		sscanf(str, "%llx", &ull);
		lgwm = ull;
	...
	*(uint32_t *)(buff_up + 8) = net_mac_l;
	...
	while (!exit_sig && !quit_sig) {
		...
		for (i=0; i < nb_pkt; ++i) {
			...
			send_measures(&msg_arrived,"uplink_received");
		...
		gettimeofday(&msg_sent, NULL);

		int s = send(sock_up, (void *)buff_up, buff_index, 0);

		send_measures(&msg_sent, "uplink_sent");
	...
	*(uint32_t *)(buff_req + 8) = net_mac_l;
	...
	while (!exit_sig && !quit_sig) {
		...
		while ((int)difftimespec(recv_time, send_time) < keepalive_time) {
			...
			send_measures(&msg_arrived,"downlink_received");
			...
			send_measures(&msg_sent,"downlink_sent");
